...
void NetCardInfo::show() {
    std::cout << "适配器名称：" << info_.adapter_name << std::endl;
    std::cout << "连接名称：" << info_.friendly_name << std::endl;
    std::cout << "物理地址：" << info_.physical_addr << std::endl;
    std::cout << "IP地址：" << info_.ip << std::endl;
    std::cout << "IP掩码：" << info_.ip_mask << std::endl;
    std::cout << "默认网关：" << info_.gateway << std::endl;
    std::cout << "主用DNS：" << info_.default_dns << std::endl;
    std::cout << "备用DNS：" << info_.backup_dns << std::endl;
    std::cout << std::endl;
}
...
bool NetCardInfo::EnableDHCP()
{
    return config_->enable_dhcp();
}
