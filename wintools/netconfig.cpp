...
bool NetConfig::set_ip_config(const string& ip, const string& mask, const string& gateway)
{
...
bool NetConfig::set_ip_config(const string& ip, const string& mask)
{
	bool rt = false;
	if (!init())
		return rt;
...
bool NetConfig::set_dns(const string& default_dns, const string& backup_dns)
{
...
bool NetConfig::set_gateway(const string& gateway)
{
	bool rt = false;
	if (!init())
		return rt;
...
shared_ptr<SAFEARRAY> NetConfig::create_SAFEARRAY(const vector<string>& args)
{
	SAFEARRAY* psa = SafeArrayCreateVector(VT_BSTR, 0, ULONG(args.size()));
	long idx[] = { 0 };
	for (int i = 0; i < args.size(); i++) {
		...
		if (FAILED(SafeArrayPutElement(psa, idx, ip))) {
			return shared_ptr<SAFEARRAY>(NULL);
		}
		SysFreeString(ip);
	}
...
bool NetConfig::set_dns_base(bool is_auto, const string& default_dns, const string& backup_dns)
{
	bool rt = false;
	if (!init())
		return rt;
